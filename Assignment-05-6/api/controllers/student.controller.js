...
const getAllStudents = (req, res) => {
    ...
    if (checked.error) {
        ...
        //find docs
        Student.find().skip(checked.offset).limit(checked.count).exec((err, docs) => {
            if (err) {
...
const updateStudent = (req, res) => {
    ...
    Student.findOneAndUpdate({ _id: req.params.studentId }, req.body, { new: true, useFindAndModify: false }, (err, doc) => {
        if (err) {
            res.status(500).json(err);
            return;
        }
